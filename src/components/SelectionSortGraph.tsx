...
export function Graph() {
  ...
  const SelectionSort = async () => {
    ...
    for (let i = 0; i < n - 1 && playRef.current; i++) {
      ...
      for (let j = i + 1; j < n && playRef.current; j++) {
        setComparisons((prev) => prev + 1);
        if (sortedData[j].height < sortedData[min_index].height) {
          min_index = j;
        }
      }
  
      if (min_index !== i) {
        [sortedData[i], sortedData[min_index]] = [sortedData[min_index], sortedData[i]];
